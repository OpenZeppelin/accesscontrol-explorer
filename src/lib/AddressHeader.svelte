<script lang="ts">
  ...
  export let view: 'account' | 'contract';
</script>

<header class="flex gap-x-2 text-lg items-baseline">
  <a class:selected={view === 'account'} href="/{$chain.shortName}:{address}/account">Account</a>
  <span>·</span>
  <a class:selected={view === 'contract'} href="/{$chain.shortName}:{address}/contract">Contract</a>
